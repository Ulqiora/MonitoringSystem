...
int main() {
    char command[]="iostat -w 1 -c 4 disk0 ";
    ...
    // double disk_size = static_cast<double>(fs_info.f_bsize * fs_info.f_blocks) / (1024 * 1024 * 1024); //
    // размер диска в Гб double free_space = static_cast<double>(fs_info.f_bsize * fs_info.f_bfree) / (1024 *
    // 1024 * 1024); // свободное место в Гб double used_space = disk_size - free_space; // занятое место в
    // Гб double io_time = 1.0 / (fs_info.f_iosize / 1000.0 / 1000.0); // время на I/O операции в секундах
    ...
    // IAgent* agent=new MemoryAgent(bd);
    // agent->update();
    // agent->update();
